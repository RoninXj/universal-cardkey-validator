...
async function loadCardKeyModule() {
    ...
    try {
        ...
        console.log('�?�?GitHub 下载失败:', error.message);
        
        // 3. 尝试使用过期的缓�?
        const cachedCode = readCache();
        if (cachedCode) {
            console.log('⚠️ 使用过期的缓存模�?);
            try {
                const module = { exports: {} };
                eval(cachedCode);
                return module.exports;
            } catch (error) {
                console.log('�?缓存模块也无法加�?);
            }
        }
...
修改你的脚本文件（例�?`ksjsb-with-cardkey.js`）：
...
直接�?`cardkey-validator.js` 的代码复制到脚本开头：
...
| **长期维护项目** | 方案1（依赖管理）�?| 版本控制清晰，符合最佳实�?|
| **网络稳定环境** | 方案1（依赖管理）�?| 标准�?npm 依赖管理方式 |
| **快速测试开�?* | 方案2（动态加载） | 无需预先安装，快速部�?|
| **单个脚本项目** | 方案2（动态加载） | 脚本相对独立，便于分�?|
| **网络不稳�?* | 方案3（内嵌代码） | 完全离线运行，最稳定 |
| **网络受限环境** | 方案3（内嵌代码） | 无需外部资源，完全独�?|
...
1. **环境变量必需**：无论使用哪种方案，都必须设�?`CARD_KEY` �?`km` 环境变量
2. **GitHub 访问**：方�?和方�?需要确保青龙面板能访问 GitHub
3. **版本管理**：建议使用版本标签（�?`#v1.0.0`），便于版本控制和回�?
4. **网络稳定�?*：方�?和方�?首次使用需要网络连�?
